...
exist, run the `16-eyes-init` skill now** (its auto-bootstrap note applies — no
...
For EACH selected lens, invoke `/16-eyes-worker`: "Run this investigation lens across
the ENTIRE repository: <paste the lens's own prompt verbatim>. Return a JSON object:
`{ findings: [ { title, file, line, description, initial_impact, initial_probability },
... ] }` — every finding anchored to a real file:line you actually read."
...
For EACH de-duplicated finding, invoke `/16-eyes-worker`: "You are adversarially
...
For each `real` finding with `impact: "high"`, invoke `/16-eyes-worker` this many times
(3 for `depth: thorough`, 1 for `depth: quick`, or `adversarial.votesPerFinding` from
config if set): "Try to REFUTE this security finding (it already passed one
