...
`.16-eyes/lenses.json`). **If it doesn't exist, run the `16-eyes-init` skill now**
(auto-bootstrap — no interview) and continue with what it produces. This skill never
designs its own lenses — no diff alone gives enough repo-wide context to do that well;
it always reuses whatever `init` already produced.

## 4. Investigate — one delegation per lens, scoped to the diff

For EACH persisted lens, invoke `/16-eyes-worker`: "You are reviewing ONLY a diff
...
`16-eyes-audit`'s job, not this skill's.
